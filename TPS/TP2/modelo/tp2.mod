...
var tcc {CENTROS, CENTROS, MEDIOS} >= 0;
...
s.t. trasbordo{a in CENTROS}: 
    sum{m in MEDIOS, o in ORIGENES} toc[o, a, m] + sum{m in MEDIOS, c in CENTROS} tcc[c, a, m] =
    sum{m in MEDIOS, c in CENTROS} tcc[a, c, m] + sum{m in MEDIOS, d in DESTINOS} tcd[a, d, m];

# Capacidad de un centro de distribución
s.t. capacidad_centro{a in CENTROS, m in MEDIOS}:
    sum{o in ORIGENES} toc[o, a, m] + sum{c in CENTROS} tcc[c, a, m] <= CC[a, m];
...
s.t. limite_dinero:
        ...
        sum{m in MEDIOS, c in CENTROS, c2 in CENTROS} CCC[c, c2, m] * tcc[c, c2, m] +
